...
def make_index_series(final_return: float, n_points: int) -> np.ndarray:
    """
    Return index levels starting at 100 and compounding smoothly to 100*(1+final_return) across n_points.
    """
...
def slice_by_range(df: pd.DataFrame, range_key: str) -> pd.DataFrame:
    """
    Returns a sliced df by range: '3M','6M','1Y','2Y','YTD','All'
    Assumes df has a 'date' column (datetime) and is sorted.
    """
    if df.empty: return df.copy()
    end = df["date"].max()

    if range_key == "All": return df.copy()
    if range_key == "YTD":
...
def build_leaderboard(df_all: pd.DataFrame, period: str) -> pd.DataFrame:
    ...
    base_cols = ["Item","Subtype","Condition","Grade","Release Year","Start ($)","Latest ($)","ROI (%)","CAGR (%)"]
    if df_all.empty:
...
def render_news(category_name: str):
    ...
    for item in cleaned:
        ...
        st.markdown(f"- [{item.get('title','Untitled')}]({item.get('link','#')})  \n"
                    f"  <span class='ri-muted' style='font-size:12px;'>{pub}</span>", unsafe_allow_html=True)

# ================ Charts & Export helpers ================
def show_item_chart(title: str, df_source: pd.DataFrame):
    """df_source: DataFrame with ['date','price_usd']"""
...
def build_export_zip(category_slug: str,
    ...
    combo_raw = pd.concat(
        ...
        ignore_index=True
    ) if (not top3_raw.empty or not bot3_raw.empty) else pd.DataFrame()
...
def render_category_tab(category_name: str, csv_path: str, news_key: str):
    ...
    if df_win.empty:
        ...
        if df_norm.empty:
            ...
            bench_series = pd.Series(
                ...
                index=cat_series.index, name=bench_key
            )
        ...
        if df_lb.empty:
            ...
            top3 = top3_raw.copy(); bot3 = bot3_raw.copy()
            ...
            with lcol:
                ...
                st.dataframe(top3[["Item","Subtype","Condition","Grade","Release Year","Start ($)","Latest ($)","ROI (%)","CAGR (%)"]])
            with rcol:
                st.caption("Bottom 3 by ROI")
                st.dataframe(bot3[["Item","Subtype","Condition","Grade","Release Year","Start ($)","Latest ($)","ROI (%)","CAGR (%)"]])
    ...
    # Export All (ZIP) — placed *after* per-item so choice/df_one exist
    ...
    if uploaded is not None:
        try:
            ...
            if "date" not in df.columns or "price_usd" not in df.columns:
                level="error"; msgs.append("Missing required columns: 'date' and/or 'price_usd'.")
            else:
                try:
                    df["date"] = pd.to_datetime(df["date"])
                    ...
                    level="error"; msgs.append(f"Date parsing error: {e}")
            if level=="error": st.error("Validation failed.")
            elif msgs:         st.warning("Validation warnings.")
            else:              st.success("Validation OK.")
            for m in msgs: st.write("•", m)
            if "date" in df.columns and "price_usd" in df.columns:
                st.markdown("**Preview (first & last 5 rows)**")
                st.dataframe(df.head(5)); st.dataframe(df.tail(5))
        except Exception as e:
