...
def build_leaderboard(df_all: pd.DataFrame, period: str) -> pd.DataFrame:
    """
    Generic leaderboard for any category dataframe shaped like:
    ['item_name','date','price_usd', plus optional metadata columns]
    Returns per-item ROI (and CAGR when possible) for the chosen window.
    """
    if df_all.empty:
        return pd.DataFrame(columns=[
            "Item","Subtype","Condition","Grade","Release Year",
    ...
    for name, g in df_all.groupby("item_name", sort=True):
        ...
        # Keep metadata from the latest row for display
        meta = g.iloc[-1]
...
def show_item_chart(title: str, df_source: pd.DataFrame):
    """
    df_source: DataFrame with columns ['date','price_usd'] (sorted or not)
    """
    ...
    st.line_chart(df.set_index("date")["price_usd"], width="stretch")
...
def render_category_tab(category_name: str, csv_path: str, news_key: str):
    ...
    csv_path: path to category dataset (expects columns: item_name,date,price_usd, +metadata)
    news_key: string for news feed key ("Toys","Watches","Cards")
    """
    st.markdown(
        f"<p style='text-align:center; color:#555;'>Tracking monthly median resale for top {category_name.lower()} by ROI (demo dataset).</p>",
        unsafe_allow_html=True
    )
    ...
    if df_win.empty:
        ...
        if df_norm.empty:
            ...
            bench_series = pd.Series(make_index_series(bench_ytd, len(cat_series)), index=cat_series.index, name=bench_key)

            chart_df = pd.DataFrame({
                f"{category_name} (Category Index)": cat_series,
                bench_key: bench_series.values
            })
    ...
    # ---------- Divider then Per-item drilldown ----------
    st.markdown("---")
...
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; font-size:16px; color:#555;'>The Rare Index is a demo platform showcasing how alternative assets like trading cards, watches, and toys can be tracked against traditional markets.</p>", unsafe_allow_html=True)
st.markdown("---")
...
# -------------------------
with tab_cards:
    # Prefer a category dataset if present; else fallback to your single demo CSV
    cards_path = "data/cards/cards_top50.csv"
    try:
        render_category_tab("Cards", cards_path, "Cards")
    except Exception:
        # Fallback: if there is no category file, show the legacy single demo if available
...
# -------------------------
# Live eBay (beta) — unchanged logic, simplified display
# -------------------------
with tab_live:
...
# -------------------------
# Top 10 (Demo) — kept minimal
# -------------------------
with tab_top10:
    st.markdown("### 🏆 Top 10 ROI (Demo)")
    st.caption("This is a lightweight demo using any 'data/cards/cards_*.csv' files plus two sample rows.")

    # Auto-discover card CSVs
    ...
    # Add two non-card demo rows (point to placeholder files if you want)
    items = items + [
    ...
    def _calc_roi(csv_path):
        try:
            ...
            start = float(df["price_usd"].iloc[0]); latest = float(df["price_usd"].iloc[-1])
